...
public final class ValueEditorMenu {
    ...
    public static void handleClick(PurrtechPVE plugin, Player player, ValueEditorHolder holder, int slot) {
        ...
        if (delta != null) {
            CurrentState state = currentState(plugin, holder);
            applyValue(plugin, holder, state.value() + delta, state.visible());
        ...
        switch (slot) {
            case VISIBLE_TOGGLE_SLOT -> {
                CurrentState state = currentState(plugin, holder);
                applyValue(plugin, holder, state.value(), !state.visible());
                render(plugin, holder.getInventory(), holder, locale);
            }
    ...
    private record CurrentState(double value, boolean visible) {
    }
    ...
    private static CurrentState currentState(PurrtechPVE plugin, ValueEditorHolder holder) {
        ...
        return switch (holder.kind()) {
            case RESIST -> service.typeModifiers(key).stream()
                    ...
                    .map(m -> new CurrentState(m.percent(), m.visible())).orElse(new CurrentState(0, true));
            case ARMOR_PENETRATION -> service.armorPenetration(key).stream()
                    ...
                    .map(p -> new CurrentState(p.amount(), p.visible())).orElse(new CurrentState(0, true));
            case ATTRIBUTE -> {
                ...
                yield service.attributeModifiers(key).stream()
                        ...
                        .map(a -> new CurrentState(a.amount(), a.visible())).orElse(new CurrentState(0, true));
            }
            case BLEED_CHANCE -> service.bleedEffect(key)
                    .map(b -> new CurrentState(b.chancePercent(), b.visible())).orElse(new CurrentState(0, true));
            case BLEED_DURATION -> service.bleedEffect(key)
                    .map(b -> new CurrentState(b.durationSeconds(), b.visible())).orElse(new CurrentState(0, true));
            case CRIT_CHANCE -> service.criticalEffect(key)
                    .map(c -> new CurrentState(c.chancePercent(), c.visible())).orElse(new CurrentState(0, true));
            case CRIT_BONUS -> service.criticalEffect(key)
                    .map(c -> new CurrentState(c.bonusDamagePercent(), c.visible())).orElse(new CurrentState(0, true));
        };
    }

    private static void applyValue(PurrtechPVE plugin, ValueEditorHolder holder, double newValue, boolean visible) {
        ItemTemplateService service = plugin.getItemTemplateService();
        String key = holder.templateKey();
        switch (holder.kind()) {
            ...
            case BLEED_CHANCE -> {
                BleedEffect current = service.bleedEffect(key).orElse(new BleedEffect(0, 0, true));
                service.setBleedEffect(key, newValue, current.durationSeconds(), visible);
            }
            case BLEED_DURATION -> {
                BleedEffect current = service.bleedEffect(key).orElse(new BleedEffect(0, 0, true));
                service.setBleedEffect(key, current.chancePercent(), newValue, visible);
            }
            case CRIT_CHANCE -> {
