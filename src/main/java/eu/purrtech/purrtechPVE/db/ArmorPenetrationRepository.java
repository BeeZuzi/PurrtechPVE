...
public final class ArmorPenetrationRepository {
    ...
    public void upsert(UUID templateId, ArmorPenetration penetration) {
        try (Connection connection = database.getConnection();
             PreparedStatement statement = connection.prepareStatement("""
                     INSERT OR REPLACE INTO item_armor_penetration (template_id, armor_class, amount, mode, visible)
                     VALUES (?,?,?,?,?)
                     """)) {
            ...
            statement.setString(4, penetration.mode().name());
            statement.setBoolean(5, penetration.visible());
    ...
    public List<ArmorPenetration> findByTemplate(UUID templateId) {
        ...
        try (Connection connection = database.getConnection();
             PreparedStatement statement = connection.prepareStatement("""
                     SELECT armor_class, amount, mode, visible FROM item_armor_penetration WHERE template_id = ?
                     """)) {
            statement.setString(1, templateId.toString());
            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
                    out.add(new ArmorPenetration(ArmorClass.valueOf(rs.getString("armor_class")), rs.getDouble("amount"),
                            DamageMode.valueOf(rs.getString("mode")), rs.getBoolean("visible")));
                }
            }
        } catch (SQLException e) {
